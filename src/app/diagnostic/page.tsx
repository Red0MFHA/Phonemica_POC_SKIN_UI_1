...
export default function DiagnosticPage() {
  ...
  return (
    <div className={`flex min-h-screen flex-col items-center justify-center bg-gradient-to-b ${theme.background} p-6`}>
      <div className="mb-4 text-4xl">{animalForPhoneme[exercise.targetPhoneme] ?? "🦜"}</div>
      <h1 className="text-2xl font-black text-white">Jungle Scout Test</h1>
      <p className="mt-1 text-sm text-lime-200">Say the word to find your special sounds</p>
      ...
      <div className="mt-6 text-center">
        ...
        <p className="mt-1 text-sm text-amber-300">{exercise.targetPhoneme} sound</p>
      </div>
      ...
      >
        {recording ? <Mic size={36} className="text-white" /> : busy ? <Loader2 className="animate-spin text-emerald-950" size={32} /> : <Mic size={36} className="text-emerald-950" />}
      </button>
      <p className="mt-3 text-sm font-medium text-lime-200">
        {recording ? "Listening…" : busy ? "The guide is listening…" : flash === "correct" ? "Great!" : "Hold and say the word"}
      </p>

      <div className="mt-8 flex gap-1.5">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className={`h-2.5 w-2.5 rounded-full ${i < step ? "bg-amber-400" : "bg-white/30"}`} />
        ))}
      </div>
    </div>
  );
...
function ScoutLoading() {
  return (
    <div className={`flex min-h-screen flex-col items-center justify-center bg-gradient-to-b ${theme.background} text-white`}>
      ...
      <p className="mt-3 text-lime-200">Preparing the scout test…</p>
    </div>
  );
}
