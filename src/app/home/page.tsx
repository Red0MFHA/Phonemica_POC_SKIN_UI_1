  ...
  return (
    <div className={`flex min-h-screen flex-col items-center bg-gradient-to-b ${theme.background} p-6`}>
      <div className="mt-6 text-center">
        <div className="text-5xl">{animalForPhoneme[child.targets[0] ?? "/r/"] ?? "🐰"}</div>
        <h1 className="mt-2 text-2xl font-black text-white">Welcome, {child.name}!</h1>
        <p className="mt-1 text-lime-200">The jungle is waiting for a brave explorer.</p>
      </div>

      <div className="mt-8 w-full max-w-sm rounded-2xl bg-emerald-950/60 p-5 text-center backdrop-blur">
        <p className="text-sm font-medium text-lime-200">Today&apos;s objective</p>
        <p className="mt-1 text-lg font-bold text-white">Reach the Ancient Temple 🗿</p>
      ...
      <button
        onClick={begin}
        disabled={busy}
        className="mt-10 rounded-2xl bg-amber-400 px-10 py-3 text-lg font-bold text-emerald-950 shadow-lg transition-transform hover:scale-105 disabled:opacity-50"
      >
