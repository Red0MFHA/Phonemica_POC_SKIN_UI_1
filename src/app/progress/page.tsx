...
const MASTERY_BADGE: Record<string, { label: string; color: string }> = {
  ...
  developing: { label: "Developing", color: "bg-amber-400" },
...
export default function ProgressPage() {
  ...
  return (
    <div className={`flex min-h-screen flex-col items-center bg-gradient-to-b ${theme.background} p-6`}>
      <h1 className="mt-2 text-2xl font-black text-white">Your Jungle Progress</h1>
      <p className="text-sm text-lime-200">Master sounds to unlock new paths</p>
        ...
        <>
          ...
          <div className="mt-6 w-full max-w-sm space-y-3">
            {progress.phonemes.map((p) => {
              ...
              return (
                <div key={p.phoneme} className="flex items-center gap-3 rounded-2xl bg-emerald-950/60 p-4">
                  <div className="text-3xl">{animalForPhoneme[p.phoneme] ?? "🐾"}</div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-white">{p.phoneme}</span>
                      <span className={`rounded-full px-2 py-0.5 text-xs font-bold text-emerald-950 ${badge.color}`}>{badge.label}</span>
                    </div>
                    <div className="mt-2 h-2.5 overflow-hidden rounded-full bg-white/20">
                      <div className={`h-full rounded-full ${badge.color}`} style={{ width: `${p.accuracy}%` }} />
                    </div>
                    <p className="mt-1 text-xs text-lime-200">{p.accuracy}% accuracy</p>
                  </div>
                </div>
              );
            })}
          ...
          <button
            ...
            className="mt-8 w-full max-w-sm rounded-2xl bg-amber-400 px-6 py-3 text-lg font-bold text-emerald-950 shadow-xl transition-transform hover:scale-105"
          >
            Back to base
          </button>
        </>
