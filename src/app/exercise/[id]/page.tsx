    ...
    <div className={`relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-gradient-to-b ${theme.background} p-6`}>
      ...
      <div className="relative z-10 flex min-h-screen w-full flex-col items-center justify-center">
        <div className="absolute top-4 left-0 right-0 flex items-center justify-center gap-1.5">
          {Array.from({ length: EXERCISES_PER_LEVEL }).map((_, i) => (
            <div key={i} className={`h-3 w-3 rounded-full transition-all ${i < done ? "scale-125 bg-amber-400" : "bg-white/30"}`} />
          ))}
        </div>
        <p className="mb-3 text-sm font-bold text-lime-200">{zone}</p>

        <div className={`text-7xl drop-shadow-lg ${recording ? "animate-bob" : "animate-floaty"}`}>
          {animalForPhoneme[exercise.targetPhoneme] ?? "🐰"}
        </div>
        <p className="mt-4 text-lg text-lime-200">{exercise.prompt ?? "Say the word"}</p>

        <div className="mt-2 text-6xl font-black tracking-wide text-white uppercase drop-shadow-lg">{exercise.word}</div>
        <p className="mt-2 text-base font-semibold text-amber-300">{exercise.targetPhoneme}</p>
        ...
        <div className="relative mt-8">
          ...
          <button
            ...
            className={`relative flex h-28 w-28 items-center justify-center rounded-full shadow-2xl transition-transform ${
              recording ? "scale-110 bg-red-500" : "bg-amber-400 hover:scale-105"
            } ${busy ? "opacity-60" : ""}`}
          >
            {recording ? <Mic size={44} className="text-white" /> : busy ? <Loader2 className="animate-spin text-emerald-950" size={36} /> : <Mic size={44} className="text-emerald-950" />}
          </button>
        </div>
        <p className="mt-4 text-base font-bold text-lime-200">
          {recording ? "Listening… keep talking! 👂" : busy ? "The engine is checking… 🕵️" : "Tap & say the word"}
        </p>
      </div>
      ...
      {result && (
        <div className={`fixed inset-0 z-30 flex items-center justify-center bg-black/60 p-6`} onClick={() => advance(result.retry)}>
          ...
          <div className={`relative w-full max-w-md rounded-3xl bg-emerald-900 p-8 text-center shadow-2xl ${result.correct ? "animate-pop-in" : ""}`}>
            {result.correct ? (
              <>
                ...
                <p className="mt-2 text-lg text-lime-200">{result.word} — spot on!</p>
              ...
              <>
                <div className={`mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-amber-500 text-white animate-wiggle`}><RotateCcw size={36} /></div>
                <h2 className="mt-4 text-3xl font-black text-white">Almost there! 💪</h2>
                <p className="mt-2 text-lg text-lime-200">Let&apos;s try this word again.</p>
                <p className="mt-3 text-amber-300">{result.accuracy}% match</p>
              </>
            )}
            <button
              onClick={(e) => { e.stopPropagation(); advance(result.retry); }}
              className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-amber-400 px-6 py-4 text-2xl font-bold text-emerald-950 shadow-lg transition-transform hover:scale-105 active:scale-95"
            >
