    ...
    <div className={`relative flex min-h-screen flex-col items-center justify-center bg-gradient-to-b ${theme.background} p-6`}>
      <div className="absolute top-4 left-0 right-0 flex items-center justify-center gap-1.5">
        {Array.from({ length: EXERCISES_PER_LEVEL }).map((_, i) => (
          <div key={i} className={`h-2.5 w-2.5 rounded-full ${i < done ? "bg-amber-400" : "bg-white/30"}`} />
        ))}
      </div>
      <p className="mb-2 text-xs font-medium text-lime-200">{zone}</p>

      <div className="text-5xl">{animalForPhoneme[exercise.targetPhoneme] ?? "🐰"}</div>
      <p className="mt-3 text-sm text-lime-200">{exercise.prompt ?? "Say the word"}</p>

      <div className="mt-2 text-5xl font-black tracking-wide text-white uppercase">{exercise.word}</div>
      <p className="mt-1 text-sm text-amber-300">{exercise.targetPhoneme}</p>
      ...
      >
        {recording ? <Mic size={38} className="text-white" /> : busy ? <Loader2 className="animate-spin text-emerald-950" size={32} /> : <Mic size={38} className="text-emerald-950" />}
      </button>
      <p className="mt-3 text-sm font-medium text-lime-200">
        {recording ? "Listening…" : busy ? "Engine is checking…" : "Hold to speak"}
      </p>

      {result && (
        <div className={`fixed inset-0 z-20 flex items-center justify-center bg-black/60 p-6`} onClick={() => advance(result.retry)}>
          <div className="w-full max-w-sm rounded-3xl bg-emerald-900 p-8 text-center shadow-2xl">
            {result.correct ? (
              <>
                ...
                <p className="mt-1 text-sm text-lime-200">{result.word} — spot on!</p>
                <p className="mt-3 text-amber-300">★ +{result.accuracy >= 85 ? "3" : "2"} stars</p>
              </>
            ) : (
              <>
                <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-amber-500 text-white"><RotateCcw size={30} /></div>
                <h2 className="mt-4 text-2xl font-black text-white">Almost there!</h2>
                <p className="mt-1 text-sm text-lime-200">Let&apos;s try this word again.</p>
                <p className="mt-3 text-amber-300">{result.accuracy}% match</p>
              </>
            )}
            <button
              onClick={(e) => { e.stopPropagation(); advance(result.retry); }}
              className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-amber-400 px-6 py-3 text-lg font-bold text-emerald-950"
            >
